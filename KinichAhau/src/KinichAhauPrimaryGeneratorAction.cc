...
//  fGunMessenger = new KinichAhauPrimaryGeneratorMessenger(this);
  ...
  while(con>0)
  {
    ...
    G4float alfa = (std::cos(theta))*(std::cos(theta))*(std::cos(theta))*(std::cos(theta));
...
G4cout << gunpos << G4endl;
  ...
  fParticleGun->SetParticleMomentumDirection(gundir);
...
void KinichAhauPrimaryGeneratorAction::GeneratePrimaries(G4Event* anEvent)
{
...
void KinichAhauPrimaryGeneratorAction::SetOptPhotonPolar()
{
...
void KinichAhauPrimaryGeneratorAction::SetOptPhotonPolar(G4double angle)
{
